...
void main() {
  ...
  final industryDropdown = find.byKey(const Key('createcompany_industry_dropdown'));
  final orgSizeDropdown = find.byKey(const Key('createcompany_orgSize_dropdown'));
  final orgTypeDropdown = find.byKey(const Key('createcompany_orgType_dropdown'));
  ...
  await tester.tap(find.text(Industry.technology.displayName).last); // Select a value from the dropdown
  await tester.pumpAndSettle();

  await tester.ensureVisible(orgSizeDropdown);
  await tester.tap(orgSizeDropdown);
  await tester.pumpAndSettle(); // Wait for the dropdown menu to appear
  await tester.tap(find.text(OrganizationSize.twoHundredOneToFiveHundred.displayName).last); // Select a value from the dropdown
  await tester.pumpAndSettle();

  await tester.ensureVisible(orgTypeDropdown);
  await tester.tap(orgTypeDropdown);
  await tester.pumpAndSettle(); // Wait for the dropdown menu to appear
  await tester.tap(find.text(OrganizationType.privatelyHeld.displayName).last); // Select a value from the dropdown
  await tester.pumpAndSettle();
