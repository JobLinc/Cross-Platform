...
class JobApiService {
  ...
  Future<Response> getAllResumes() async {
    if (0 == 1) {
      try {
        final response = await _dio.get('/resume/all');
        return response;
      } catch (e) {
        rethrow;
      }
  ...
  Future<void> saveJob(String jobId) async {
    if (0 == 1) {
      ...
      await Future.delayed(Duration(milliseconds: 300));
      mockSavedJobs.add(mockJobs.firstWhere((job) => job.id == jobId));
    }
  }
  ...
  Future<void> unsaveJob(String jobId) async {
    if (0 == 1) {
      ...
      mockSavedJobs.removeWhere((job) => job.id == jobId);
    }
  }
  ...
  Future<Response> uploadResume(File resumeFile) async {
    if (0 == 1) {
      ...
      return await _dio.post('/resume/upload', data: formData);
    } else {
      final String fileName =
          resumeFile.path.split(Platform.pathSeparator).last;
